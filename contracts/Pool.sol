...
contract Pool is Third {
    ...
    function updatePool(uint256 _pid,uint256 _amount,bool isAdd) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 OFIReward = multiplier.mul(OFIPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 miningReward = 38500000000000000000;
        uint256 devReward = miningReward.mul(1875);
        OFI.mint(devaddr, devReward.div(10000)); // 15% Development
        uint256 oprReward = miningReward.mul(375);
        OFI.mint(operationaddr, oprReward.div(10000)); // 3% Operation
        OFI.mint(address(this), OFIReward); // Liquidity reward
    ...
    function safeOFITransfer(address _to, uint256 _amount) internal {
        ...
        uint256 ba = OFI.balanceOf(address(this));
