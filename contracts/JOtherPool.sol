...
contract JOtherPool is Third {
    ...
    uint256 public fee = 50; // 30% of profit
    ...
    function rewardLp(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if(thirdPool.userInfo(address(this)).amount <= 0){
            return 0;
        }
        uint256 ba = getWithdrawBalance(_pid, userShares[_pid][_user], thirdPool.userInfo( address(this)).amount);
        if(ba > uRIT.amount){
    ...
    function allRewardLp(uint256 _pid) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        if(thirdPool.userInfo(address(this)).amount<=pool.lpSupply){
            return 0;
        }
        return pool.allWithdrawReward.add(thirdPool.userInfo(address(this)).amount.sub(pool.lpSupply));
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) { // 
            ...
            uint256 _before = thirdPool.userInfo(address(this)).amount;
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            thirdPool.deposit(_amount);
            ...
            uint256 _after = thirdPool.userInfo(address(this)).amount;
    ...
    function safeWithdraw(uint256 _pid) public onlyOwner{
        ...
        PoolInfo storage pool = poolInfo[_pid];
        thirdPool.withdraw(pool.lpSupply);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            uint256 fene = thirdPool.userInfo(address(this)).amount;
            ...
            thirdPool.withdraw(should_withdraw); // 
